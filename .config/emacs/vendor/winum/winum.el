...
(defun winum--update ()
  ...
  (let ((windows (winum--window-list)))
    ...
    (when winum-assign-functions
      (mapcar #'winum--try-to-find-custom-number windows ))
...
first number anyway."
  (with-selected-window window
    (with-current-buffer (window-buffer window)
      (let* ((nums (thread-last winum-assign-functions
                                (mapcar (lambda (it) (cons it (funcall it))))
                                (cl-remove-if-not (lambda (it) (null (cdr it))))))
             (num (thread-first nums (cl-first) (cdr))))
        (when (> (length nums) 1)
          (message "Winum conflict - window %s was assigned a number by multiple custom assign functions: '%s'"
                   window (mapcar (lambda (it) (format "%s -> %s" (car it) (cdr it)) nums))))
